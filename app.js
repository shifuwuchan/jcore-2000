...
function onTimeout() {
  ...
  if (state.curWord && state.curWord.id) updateSRSQuiz(state.curWord.id, false);
...
function checkAnswer(btn, isCorrect) {
  ...
  // Mise à jour SRS
  if (state.curWord && state.curWord.id) updateSRSQuiz(state.curWord.id, isCorrect);
  ...
  if (isCorrect) {
    ...
    // Classement : ces points comptent pour le total à vie (jamais remis à 0)
...
/* ══════════════════════════════════════════════════════════════════════
   MODE FLASHCARD (Anki)
   buildFlashcardQueue (srs.js) gère le pool actif limité : les nouveaux
   mots n'entrent qu'au fur et à mesure que d'autres deviennent "acquis"
   (mature). Les mots acquis ne reviennent que rarement, mélangés aux
   autres. "Black-out" (je ne sais pas du tout) boucle serrée immédiate,
   distinct d'un simple "Difficile".
══════════════════════════════════════════════════════════════════════ */
...
function startFlashSession(wordList) {
  ...
  const queue = buildFlashcardQueue(wordList); // srs.js
  if (queue.length === 0) {
...
function flipFlashCard() {
  ...
  ['blackout', 'hard', 'medium', 'easy'].forEach(ease => {
    ...
    if (el) el.textContent = previewIntervalLabel(w.id, ease); // srs.js
  });
 ...
 * "blackout" → boucle serrée (revient 1–3 cartes plus loin).
 * "hard"     → revient dans la session, plus loin (3–6 positions).
...
function rateFlashCard(ease) {
  ...
  if (ease === 'blackout') {
    reinsertInSession(state.flashQueue, state.flashIndex, state.flashCurrent, 1, 3); // srs.js
  } else if (ease === 'hard') {
    reinsertInSession(state.flashQueue, state.flashIndex, state.flashCurrent, 3, 6); // srs.js
  }
...
function finishFlashSession() {
  ...
  `;
  document.getElementById('done-rank').textContent = `${getDueWords().length} mots encore dus aujourd'hui`;
